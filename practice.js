...
function divider(numbersArray) {
  ...
  for (let number of numbersArray) {
    number % 2 == 0 ? newArray[0].push(number) : newArray[1].push(number)
  }
  return newArray
}
...
function finder(array) {
  ...
  return array.indexOf(randomNum) != -1 ? true : false
}
...
function removeItem(myGroceryList, item) {
  return !myGroceryList ? [] : myGroceryList.filter(groceryItem => groceryItem != item)
...
users = users.filter(user => user.name != "Tyler McGinnis")
